...
public class TramLineDrawer : MonoBehaviour {
    ...
    private List<Vector2> triggerPoints = new List<Vector2>();
    ...
    void Start()
    {
        gameObject.layer = 3; // 마우스 레이케스트에 안닿을려고
    ...
    void Update()
    {
        if (Input.GetMouseButton(0)) //Or use GetKey with key defined with mouse button
        {
            ...
            {
                if (hit.collider == null || hit.collider.name == "TramLineTrigger") // 다른 옵젝이랑 충돌이 없으면 그냥 그림
                {
        ...
        {
            //if (total_Length < 1)//최소 길이 제한1을 못넘으면 선 삭제
            //{
            //    Destroy(gameObject);
            //    enabled = false;
            //    return;
    ...
    void addCollider() // 콜라이더 추가
    {
        ...
        gameObject.transform.GetChild(0).SendMessage("makeTrigger",colliderPoints);
    }
